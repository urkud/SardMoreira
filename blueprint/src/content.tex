...
\begin{proof}
  ...
  Otherwise, we choose any \(\epsilon > 0\) (e.g., \(\epsilon = 1\)).
\end{proof}
...
\begin{lemma}%
  ...
  \uses{def:cdh-at}
  \lean{ContDiffMoreiraHolderAt.prodMk}
  \leanok%
...
\begin{lemma}%
  ...
  \uses{def:cdh-at}
  \lean{ContDiffMoreiraHolderAt.continuousLinearMap_comp}
  \leanok%
...
\begin{lemma}%
  ...
  \uses{def:cdh-at}
  \lean{ContDiffMoreiraHolderAt.fderiv}
  \leanok%
...
\begin{proof}
  \leanok%
  The lemma immediately follows from \(D^{k+1}f = D(D^{k}f)\).
\end{proof}

\begin{corollary}%
  \label{lem:cdh-at-iteratedFDeriv}
  \uses{def:cdh-at}
  \lean{ContDiffMoreiraHolderAt.iteratedFDeriv}
  \leanok%
...
\begin{lemma}%
  ...
  \uses{def:cdh-at}
  \lean{ContDiffMoreiraHolderAt.comp}
  \leanok%
...
\begin{lemma}%
  ...
  \lean{ContDiffMoreiraHolderOn.exists_superset}
  \leanok%
...
\begin{proof}
  ...
  \paragraph{Case 1: \(f\) is \(C^{k+(\alpha)}\) at each \(a \in K\).}
  In this case, for each \(a \in K\) choose an open neighborhood \(U_{a}\) such that \(f\) is \(C^{k}\) on \(U_{a}\).
  Then \(f\) is \(C^{k}\) on the union \(U = \bigcup_{a\in K} U_{a} \superset K\), hence it's \(C^{k+(\alpha)}\) on \((K, U)\).
  So, in this case both parts of the equivalence are true.

  \paragraph{Case 2: \(f\) is not \(C^{k+(\alpha)}\) at some \(a \in K\).}
  In this case, \(f\) is not \(C^{k+(\alpha)}\) on \((K, U)\) for any \(U\), so we can take \(U\) to be the whole space.
\end{proof}

\begin{lemma}%
  \label{lem:cdh-on-comp}
  \uses{def:cdh-on}
  \lean{ContDiffMoreiraHolderOn.comp}
  \leanok%
  Suppose that
  \begin{itemize}
...
\begin{proof}
  ...
  and the fact that a second countable topological space is hereditarily Lindelof.
\end{proof}
