...
\begin{proof}
  ...
  \[
    {(g\circ f)}^{(k)}(a) = \sum_{P}TC_{P}\left(g^{(|P|)}(a), f^{(|P_{0}|)}(a), \dots, f^{\left(\left|P_{|P|-1}\right|\right)}(a)\right),
  \]
  ...
  we have
  \begin{align*}
    TC_{P}&\left(g^{(|P|)}(x), f^{(|P_{0}|)}(x), \dots, f^{\left(\left|P_{|P|-1}\right|\right)}(x)\right) -\\
          &TC_{P}\left(g^{(|P|)}(a), f^{(|P_{0}|)}(a), \dots, f^{\left(\left|P_{|P|-1}\right|\right)}(a)\right) =\\
          &O(\|x - a\|^{\alpha}).
  \end{align*}
\end{proof}
...
\begin{remark}
  ...
  and prove equivalence of two definitions in \autoref{lem:cdn-on-nhds-iff-open}.
\end{remark}
